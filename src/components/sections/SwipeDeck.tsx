...
export default function SwipeDeck({ items }: { items: SwipeItem[] }) {
    ...
    () => [
      {
        ...
        shadow: "shadow-[0_0_30px_rgba(236,72,153,0.35)]",
        border: "border-pink-500/40",
      },
      {
        ring: "ring-2 ring-green-500/60",
        shadow: "shadow-[0_0_30px_rgba(34,197,94,0.35)]",
        border: "border-green-500/40",
      },
      {
        ring: "ring-2 ring-purple-500/60",
        shadow: "shadow-[0_0_30px_rgba(168,85,247,0.35)]",
  ...
  function moveDrag(x: number, y: number) {
    ...
    valuesRef.current.x = dx;
    valuesRef.current.y = dy;
    valuesRef.current.rot = dx / 20;
    schedule();
  }
  function endDrag() {
    if (!draggingRef.current) return;
    ...
    if (Math.abs(x) > threshold) {
      ...
      if (card) {
        ...
        card.style.transform = `translate3d(${dir * 800}px, ${y}px, 0) rotate(${rot + dir * 20}deg)`;
        card.style.opacity = "0";
      }
      setTimeout(() => {
        setIndex((i) => (i + 1) % items.length);
        valuesRef.current = { x: 0, y: 0, rot: 0 };
      ...
      <div className={`h-full flex flex-col ${muted ? 'opacity-85' : ''}`}>
        ...
        <div className="flex-1 p-4 sm:p-6 flex flex-col justify-between">
          ...
          <p className="text-sm sm:text-base text-gray-300 leading-relaxed mt-3">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla facilisi. Integer vitae augue in ipsum.
          </p>
        </div>
      </div>
    );
  ...
  const thirdIndex = (index + 2) % items.length;

  return (
    <div className="mx-auto w-[88vw] max-w-[480px] sm:max-w-[540px] md:max-w-[560px] relative isolate" style={{ height: "56vh" }}>
      {/* Masked stage to prevent background mixing on drag */}
      <div className="absolute inset-0 rounded-3xl overflow-hidden z-0 pointer-events-none">
        <div className="absolute inset-0 bg-black/90" />
      </div>

      {/* Preview cards */}
      <div className="absolute inset-0 rounded-3xl bg-gray-950 border border-gray-800/60 pointer-events-none z-10 opacity-70" style={{ transform: `translateY(18px) scale(0.98)` }}>
        {renderCardInner(thirdIndex, true)}
      </div>
      <div className="absolute inset-0 rounded-3xl bg-gray-950 border border-gray-800/60 pointer-events-none z-20 opacity-80" style={{ transform: `translateY(9px) scale(0.99)` }}>
        {renderCardInner(nextIndex, true)}
      </div>
